...
class InputValidator {
  static validate(inputs) {
    ...
    if (!bankNameRegex.test(bankName)) {
      alert(
        'Bank name must start with a capital letter, can contain letters and numbers (not at the start), and be up to 50 characters long.'
      );
      return false;
    }
    ...
    if (!downPaymentRegex.test(downPayment) || downPayment < 0.1 * principal) {
      alert(
        'Down payment must be a number and not less than 10% of the principal.'
      );
      return false;
    }

    if (
      !principalRegex.test(principal) ||
      principal < 1000 ||
      principal > 1000000
    ) {
    ...
    return [principal, rate, years, downPayment].every(
      (input) => input >= 0 && !isNaN(input)
    );
  }
}
...
document
  ...
  .addEventListener('submit', function (event) {
    ...
    const totalPayment = calculator
      .calculateTotalPayment(monthlyPayment)
      .toFixed(2);
    const totalInterest = calculator
      .calculateTotalInterest(totalPayment)
      .toFixed(2);
    ...
    const amortizationSchedule =
      calculator.generateAmortizationSchedule(monthlyPayment);
