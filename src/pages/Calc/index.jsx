        ...
        <Box>
          <Box sx={{ m: 5 }}>
            <FormControl fullWidth sx={{ mb: 5 }}>
              ...
              >
                {banks.map(({ id, BankName }) => (
                  <MenuItem key={id} value={BankName}>
                    {BankName}
                  </MenuItem>
                ))}
              </Select>
            ...
            <Box sx={{ display: 'flex', mx: 'auto', mt: 3, width: 350 }}>
              <Button variant="contained" onClick={submitCalc}>
                Calculate
              </Button>
              <Tooltip title="Clear">
              ...
              {error && (
                <Typography data-testid="calc-error">{error}</Typography>
              )}
              {warning && (
                <Typography data-testid="interest-rate-warning">
                  {warning}
                </Typography>
              )}
          ...
          <Box sx={{ display: 'flex', mx: 'auto' }}>
            <Paper
              elevation={3}
              sx={{ display: 'flex', mx: 'auto', width: 600, height: 128 }}
            >
              <Typography
                sx={{ display: 'flex', mx: 'auto', my: 'auto', fontSize: 22 }}
              >
                Your monthly mortgage payment{' '}
                {numeral(monthPayment).format('$0,0.00')}
              </Typography>
