  ...
  describe('BankCard Component', () => {
    ...
    test('calls addBank action with correct data on form submit', async () => {
      ...
      fireEvent.change(screen.getByLabelText(/interest rate/i), {
        target: { value: testData.interestRate.toString() },
      });
      fireEvent.change(screen.getByLabelText(/max loan/i), {
        target: { value: testData.maxLoan.toString() },
      });
      fireEvent.change(screen.getByLabelText(/min down payment/i), {
        target: { value: testData.minDownPayment.toString() },
      });
      fireEvent.change(screen.getByLabelText(/loan term/i), {
        target: { value: testData.loanTerm.toString() },
      });

      fireEvent.click(screen.getByRole('button', { name: /submit/i }));

      await waitFor(() =>
        expect(bankActions.addBank).toHaveBeenCalledWith(testData)
      );
    });
  });
});
...
describe('Async operations in BankCard Component', () => {
  test('handles loading state correctly', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('loading')).toBeEmptyDOMElement()
    );
  });
});
